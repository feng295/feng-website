...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function setupReserveParking() {
        ...
        reserveDateInput.value = today; // 2025-05-23
        const now = new Date();
        const currentHour = now.getHours().toString().padStart(2, '0');
        const currentMinute = now.getMinutes().toString().padStart(2, '0');
        startTimeInput.value = `${currentHour}:${currentMinute}`; // 當前時間 17:30
        ...
        const debouncedHandleReserveSearch = debounce(async () => {
            ...
            if (map && map.markers) map.markers.forEach(marker => marker.map = null);
            map.markers = [];
