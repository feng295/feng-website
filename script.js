...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    // ====================== 終極統一進出場系統（取代原本兩個 function）======================
    let cameraStream = null;
    let detectionTimer = null;
    let isProcessing = false;

    async function initUnifiedParking() {
        // 只給租用者用
        if (getRole() !== "renter") return;

        const section = document.getElementById("unifiedParking");
        if (!section) return;
        ...
        try {
            ...
            if (!res.ok) throw new Error("無法取得停車場清單");

            const { data } = await res.json();
            const parkings = Array.isArray(data) ? data : [];
            ...
            select.innerHTML = '<option value="" disabled>載入失敗，請重新登入</option>';
        ...
        select.addEventListener("change", () => {
            ...
            if (hasValue) {
                const isEntry = select.value.endsWith("-in");
                startBtn.textContent = isEntry ? "開始進場掃描" : "開始出場掃描";
            }
        });

        // 開始掃描
        startBtn.addEventListener("click", async () => {
            const [parkingId, action] = select.value.split("-");
            const parkingName = select.selectedOptions[0].text.split(" ─ ")[0];

            scanArea.classList.remove("hidden");
            plateDisplay.innerHTML = `<div class="text-6xl font-bold text-blue-600">掃描中...<br>${parkingName} ${action === "in" ? "進場" : "出場"}</div>`;
            ...
            try {
                ...
                resultDisplay.innerHTML = `<div class="text-red-600 text-5xl font-bold">攝影機開啟失敗</div>`;
                console.error(err);
            }
        });
        ...
        function startDetection(parkingId, action, parkingName) {
            ...
            const endpoint = action === "in"
                ? "/license-plate/process_frame-rent"
                : "/license-plate/process_frame-settle";
            ...
            const capture = () => {
                ...
                canvas.toBlob(async blob => {
                    ...
                    try {
                        ...
                        if (plate && plate !== "尚未檢測到車牌") {
                            ...
                            plateDisplay.innerHTML = `<div class="text-9xl font-black text-green-600 animate-pulse">${formattedPlate}</div>`;

                            if (action === "in") {
                                await doEntry(formattedPlate, parkingId, parkingName);
                            } else {
                                await doExit(formattedPlate, parkingName);
                            }
                ...
                detectionTimer = setTimeout(capture, 1800);
            }, "image/jpeg", 0.9);
        };

detectionTimer = setTimeout(capture, 1000);
    }

// 進場
async function doEntry(plate, parkingId, name) {
    try {
        ...
        if (res.ok) {
            resultDisplay.innerHTML = `
                    <div class="text-green-600 text-8xl font-black">進場成功！</div>
                    <div class="text-6xl mt-8">${name}</div>
                    <div class="text-5xl text-gray-600 mt-10">歡迎光臨</div>
                `;
        } else {
            throw new Error("進場失敗");
        }
    } catch (err) {
        resultDisplay.innerHTML = `<div class="text-red-600 text-7xl font-black">進場失敗</div>`;
    }
}

// 出場
async function doExit(plate, name) {
    try {
        ...
        if (res.ok) {
            const amount = data.data?.total_cost ?? data.total_cost ?? 0;
            resultDisplay.innerHTML = `
                    <div class="text-green-600 text-8xl font-black">出場成功！</div>
                    <div class="text-6xl mt-8">${name}</div>
                    <div class="text-9xl font-black text-red-600 mt-12">${amount} 元</div>
                `;
        } else {
            throw new Error(data.error || "出場失敗");
        }
    } catch (err) {
        resultDisplay.innerHTML = `<div class="text-red-600 text-7xl font-black">${err.message || "出場失敗"}</div>`;
    }
}
...
function resetSystem() {
    ...
    plateDisplay.innerHTML = `<div class="text-8xl font-black text-gray-500">等待車輛進入感應區...</div>`;
    resultDisplay.innerHTML = "";
    select.value = "";
...
function showSection(sectionId) {
    ...
    if (section) {
        ...
        // 只有切到 unifiedParking 才初始化（避免重複執行）
    ...
    pageTitle.textContent = titles[sectionId] || "行動停車場管理資訊系統";
}
